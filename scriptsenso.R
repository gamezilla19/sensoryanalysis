    ...
    if(test_type == "Triangular") {
      ...
      results_db <- results_data %>%
        mutate(
          ...
          sd_value = SD,
          n_observations = N,
          anova_5pct = ifelse("ANOVA à 5%" %in% names(.), `ANOVA à 5%`, NA),
          anova_10pct = ifelse("ANOVA à 10%" %in% names(.), `ANOVA à 10%`, NA)
        ) %>%
        select(source_name, idtest, test_type, segment, segment_id, product_name,
...
analyze_products <- function(segment, segment_index, file_path = NULL, file_test_type) {
  tryCatch({
    ...
    anova_5pct <- !is.na(p_value_produit) && p_value_produit < 0.05
    anova_10pct <- !is.na(p_value_produit) && p_value_produit < 0.10
